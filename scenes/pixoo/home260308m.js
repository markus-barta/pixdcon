...
async function drawPvCons(d, cx, cy, productionW, consumptionW) {
  ...
  await drawKwTight(d, cx, cy - 6, productionW, pvColor);
  ...
  await drawKwTight(d, cx, cy + 2, consumptionW, consColor);
}
...
export default {
  ...
  async render(device) {
    ...
    // Row 0 = y 8..25 (18px). Each circle r=2 (5px). Top cy=12, bottom cy=21.
    const cx0 = COLS[0].cx;
