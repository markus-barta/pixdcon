...
//
// Layers (back→front):
//   r=3 gray outline  — outermost ring
//   r=2 gray ring     — together with r=3 forms a 2px gray outline
//   center fill       — 33% opacity status color (1px)
//   r=1 colored ring  — cross (±1,0),(0,±1)
// Locked  → full circle. Unlocked → open top (skip dy=-1 on r=1, dy=-2,-1 on r=2, dy=-3,-2 on r=3).
...
function drawNukiCircle(d, cx, cy, nukiState, alive) {
  ...
  // r=3 gray ring — outermost; open: skip top 2 rows (dy=-3,-2) = keep dy>=-1
  const ring3 = [
    [0,-3],[1,-3],[-1,-3],
    [2,-2],[-2,-2],
    [3,-1],[-3,-1],
  ...
  const p3 = isOpen && !stale ? ring3.filter(([, dy]) => dy >= -1) : ring3;
  for (const [dx, dy] of p3) d._setPixel(cx + dx, cy + dy, 40, 40, 40);

  // r=2 gray ring — inner band of the 2px outline; open: skip dy=-2,-1
  const ring2 = [
    [0,-2],[1,-2],[-1,-2],
    [2,-1],[-2,-1],
    [2,0],[-2,0],
  ...
  const p2 = isOpen && !stale ? ring2.filter(([, dy]) => dy >= 0) : ring2;
  for (const [dx, dy] of p2) d._setPixel(cx + dx, cy + dy, 40, 40, 40);

  // Center fill — 1px at 33% opacity
  d._setPixel(cx, cy, (r * 0.33) | 0, (g * 0.33) | 0, (b * 0.33) | 0);

  // r=1 colored ring — cross (±1,0),(0,±1); open: skip dy=-1
  const ring1 = [[1,0],[-1,0],[0,1],[0,-1]];
  const p1 = isOpen && !stale ? ring1.filter(([, dy]) => dy >= 0) : ring1;
  for (const [dx, dy] of p1) d._setPixel(cx + dx, cy + dy, r, g, b);
}
