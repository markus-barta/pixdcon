...
// ── Icon: Nuki circle ─────────────────────────────────────────────────────────
//
// r=4 (9px diameter) Bresenham circle.
// Locked  → full red circle.
// Unlocked → green arc, top 3 rows removed (open-top "U" shape).
...
function drawNukiCircle(d, cx, cy, nukiState, alive) {
  ...
  // Bresenham r=4 circle pixels — [dx, dy] relative to center
  const full = [
    [0,-4],[1,-4],[-1,-4],
    [2,-3],[-2,-3],[3,-3],[-3,-3],
    [3,-2],[-3,-2],
  ...
  // Open: skip top 3 rows (dy = -4, -3, -2)
  const pixels = isOpen && !stale ? full.filter(([, dy]) => dy >= -1) : full;
...
export default {
  ...
  async render(device) {
    ...
    // Row 0 = y 8..25 (18px). Each circle r=4 (9px). Top cy=12, bottom cy=21.
    const cx0 = COLS[0].cx;
