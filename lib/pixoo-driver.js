...
export class PixooDriver {
  constructor(ip, options = {}) {
    ...
    this.deviceName = options.deviceName || ip;
  ...
  /** Switch to a built-in channel. 0=Clock, 1=Cloud, 2=Visualizer, 3=Custom, 4=Animation */
  async setChannel(index) {
    try {
      await this._httpPost({ Command: "Channel/SetIndex", SelectIndex: index });
      return true;
    } catch (err) {
      this.logger.error(`[PixooDriver] setChannel failed: ${err.message}`);
      return false;
    }
  }
  ...
  async setScreen(on) {
    try {
      await this._httpPost({ Command: "Channel/OnOffScreen", OnOff: on ? 1 : 0 });
      return true;
    } catch (err) {
      this.logger.error(`[PixooDriver] setScreen failed: ${err.message}`);
      return false;
    }
  }
  ...
  static getBuiltinModes() {
    return [
      { key: "builtin:clock", label: "Clock", channel: 0 },
      { key: "builtin:cloud", label: "Cloud Channel", channel: 1 },
      { key: "builtin:visualizer", label: "Visualizer", channel: 2 },
      { key: "builtin:animation", label: "Animation", channel: 4 },
      { key: "builtin:black", label: "Black (Screen Off)" },
    ];
  }
  ...
  async push() {
    ...
    if (this.previewStore) {
      this.previewStore.capturePixoo(this.deviceName, this.buf);
    }
