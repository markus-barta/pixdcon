 ...
 * Reference: /Users/markus/Code/pixdcon/docs/AWTRIX-API.md
...
export class UlanziDriver {
  constructor(ip, options = {}) {
    ...
    this.appName = options.appName || "pixdcon";
    this.logger = options.logger || console;
    this.cleanupLegacyApps = options.cleanupLegacyApps === true;
    this._cleanupDone = false;
  }

  /**
   * Initialize and verify device is reachable. When `cleanupLegacyApps` is
   * enabled (set via the device config in config.json), the first successful
   * initialize after process start also deletes any custom apps on the
   * device's flash that aren't `this.appName`. Useful after recovering a
   * device from AP mode where AWTRIX cycles through legacy custom apps
   * (e.g. pidicon-era leftovers) until pixdcon manages to overwrite.
   */
  async initialize() {
    try {
      ...
      if (response.ok) {
        ...
        if (this.cleanupLegacyApps && !this._cleanupDone) {
          await this._cleanupNonPixdconApps();
  ...
  /**
   * List `/CUSTOMAPPS/` via the device's `/list` endpoint and POST an empty
   * body to `/api/custom?name=<X>` for every entry whose basename isn't
   * `this.appName`. Best-effort: any failure is logged warn and swallowed.
   * Source: docs/AWTRIX-API.md examples 5 + 6.
   */
  async _cleanupNonPixdconApps() {
    try {
      const list = await fetch(`${this.baseUrl}/list?dir=/CUSTOMAPPS`);
      if (!list.ok) {
        this.logger.warn(
          `[UlanziDriver] cleanup: /list returned HTTP ${list.status} — skipping`,
        );
        return;
      }
      const files = await list.json();
      const allowed = `${this.appName}.json`;
      const stale = files.filter(
        (f) =>
          f.type === "file" && f.name.endsWith(".json") && f.name !== allowed,
      );
      if (stale.length === 0) {
        this.logger.debug(
          `[UlanziDriver] cleanup: no stale custom apps on ${this.ip}`,
        );
        return;
      }
      for (const f of stale) {
        ...
        const del = await fetch(
          `${this.baseUrl}/api/custom?name=${encodeURIComponent(name)}`,
          {
            method: "POST",
        ...
        if (del.ok) {
          this.logger.info(
            `[UlanziDriver] cleanup: deleted stale custom app "${name}" on ${this.ip}`,
          );
        } else {
          this.logger.warn(
            `[UlanziDriver] cleanup: failed to delete "${name}" — HTTP ${del.status}`,
          );
        }
      }
    } catch (err) {
      this.logger.warn(`[UlanziDriver] cleanup: ${err.message}`);
    }
  }
  ...
  /** Returns available built-in modes for this device type. */
  static getBuiltinModes() {
    return [
      { key: "builtin:time", label: "Time", app: "Time" },
      { key: "builtin:date", label: "Date", app: "Date" },
      { key: "builtin:temp", label: "Temperature", app: "Temperature" },
      { key: "builtin:humidity", label: "Humidity", app: "Humidity" },
      { key: "builtin:battery", label: "Battery", app: "Battery" },
    ];
  }
}
