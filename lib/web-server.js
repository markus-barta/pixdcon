 ...
 *   POST /api/brightness      → {deviceName, enabled, value} → per-device brightness override
 */
...
import { UlanziDriver } from "./ulanzi-driver.js";
import { PixooDriver } from "./pixoo-driver.js";
...
export class WebServer {
  ...
  constructor(options = {}) {
    ...
    this.getDeviceModes = options.getDeviceModes || (() => ({}));
    this.getRenderLoops = options.getRenderLoops || (() => []);
  ...
  async _handle(req, res) {
    ...
    try {
      ...
      if (method === "GET" && url.pathname === "/tailwind.css") {
        try {
          const css = readFileSync(
            join(__dirname, "../public/tailwind.css"),
            "utf-8",
          );
          res.writeHead(200, {
      ...
      if (method === "GET" && url.pathname === "/api/scenes") {
        const meta = this.getSceneMetadata();
        this._json(res, {
          ...meta,
          builtins: {
            ulanzi: UlanziDriver.getBuiltinModes(),
            pixoo: PixooDriver.getBuiltinModes(),
          },
        });
        return;
      }
      ...
      if (method === "GET" && url.pathname === "/api/status") {
        ...
        const deviceHealth = {};
        const now = Date.now();
        for (const { device, loop } of this.getRenderLoops()) {
          const s = loop.getStatus();
          const max = s.brightnessMax || 100;
          deviceBrightness[device.name] = {
            override:
              s.brightnessOverride != null
                ? Math.round((s.brightnessOverride / max) * 100)
                : null,
            last:
              s.lastBrightness != null
                ? Math.round((s.lastBrightness / max) * 100)
                : null,
          };
          // "Offline" trips at 5 consecutive errors. More eager than the
          // render-loop's circuit-breaker (10) on purpose: the UI indicator
          // should appear within ~30s, not wait the full ~127s exponential-
          // backoff sequence (1+2+4+8+16+32+…) before showing.
          const tele = this.telemetryCollector?.getLatest(device.name);
          // AP-mode SSID: AWTRIX broadcasts the device's `uid` verbatim
          // (e.g. uid `awtrix_5807d0` → SSID `awtrix_5807d0`).
          const apSsid = tele?.uid || null;
          deviceHealth[device.name] = {
            online: s.consecutiveErrors < 5,
            consecutiveErrors: s.consecutiveErrors,
            lastSuccessAgoMs: s.lastSuccessTime
              ? now - s.lastSuccessTime
              : null,
            ip: device.ip,
            type: device.type,
            apSsid,
        ...
        this._json(res, {
          ...
          version: VERSION,
          deviceModes: this.getDeviceModes(),
          deviceBrightness,
          deviceHealth,
        });
        return;
      ...
      if (method === "POST" && url.pathname === "/api/device") {
        ...
        cfg.devices.push({ name, type, ip, scene: null });
        await writeFile(
          this.configPath,
      ...
      if (method === "POST" && url.pathname === "/api/overlay") {
        const { deviceName, scene } = JSON.parse(await this._body(req));
        if (!this.mqttService?.connected) {
          this._json(res, { error: "MQTT not connected" }, 503);
          return;
        }
        this.mqttService.publish(
          `overlay/device/${deviceName}/scene`,
          scene || "",
          true,
        );
        this._json(res, { ok: true });
      ...
      if (method === "POST" && url.pathname === "/api/scene") {
        const body = JSON.parse(await this._body(req));
        const { deviceName, scene } = body;
        if (!deviceName) {
          this._json(res, { ok: false, error: "deviceName required" }, 400);
          return;
        }
        const loops = this.getRenderLoops();
        const entry = loops.find((e) => e.device.name === deviceName);
        if (!entry) {
          this._json(
            res,
            { ok: false, error: `Device "${deviceName}" not found` },
            404,
          );
          return;
        }
        entry.loop.setScene(scene || null);

        // Update the in-memory effective config so /api/config reflects it
        // immediately, then persist to disk so the choice survives a restart
        // (PIXD-40 — this used to be memory-only, and a restart silently
        // reverted the device to whatever config.json still said).
        const config = this.getEffectiveConfig();
        const dev = config?.devices?.find((d) => d.name === deviceName);
        if (dev) dev.scene = scene;

        const cfg = JSON.parse(JSON.stringify(config));
        const persisted = cfg?.devices?.find((d) => d.name === deviceName);
        if (persisted) {
          persisted.scene = scene || null;
          await writeFile(
            this.configPath,
            JSON.stringify(cfg, null, 2) + "\n",
            "utf-8",
      ...
      if (method === "POST" && url.pathname === "/api/brightness") {
        const { deviceName, enabled, value } = JSON.parse(
          await this._body(req),
        );
        if (!this.mqttService?.connected) {
          this._json(res, { error: "MQTT not connected" }, 503);
        ...
        const payload = enabled
          ? JSON.stringify({ enabled: true, value: Math.round(value) })
          : "";
        this.mqttService.publish(
          `${deviceName}/brightness_override`,
          payload,
          true,
        );
        this._json(res, { ok: true });
  ...
  async _cloneScene({
    ...
    // When cloning (not overwriting), if a device uses the source scene,
    // we don't auto-switch it — the user can select the new scene via the dropdown.
    // No multi-scene array to update.
...
<head>
  ...
  <title>pixdcon</title>
  <link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'><rect width='32' height='32' rx='4' fill='%230066FF'/><text x='16' y='24' text-anchor='middle' font-family='sans-serif' font-weight='bold' font-size='22' fill='%23FFFFFF'>P</text></svg>">
...
<body class="min-h-screen" x-data="app()" x-init="init()" x-cloak
  x-effect="document.body.style.overflow = (showSceneSettings || showDeviceSettings || showAddDevice) ? 'hidden' : ''">
  ...
  <header class="sticky top-0 z-40 border-b border-white/5" style="background:rgba(13,17,23,0.85);backdrop-filter:blur(16px)">
    <div class="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <span class="font-semibold text-white tracking-tight">pixdcon</span>
        ...
        <div class="glass-card rounded-2xl overflow-hidden"
          ...
          <div class="px-5 pt-4 pb-3" style="border-bottom:1px solid rgba(255,255,255,0.06)">
            <div class="flex items-center justify-between gap-2 mb-1.5">
              <div class="font-semibold text-white leading-tight text-[15px]" x-text="deviceTitle(device)"></div>
              <div class="flex items-center gap-2 flex-shrink-0">
                <span x-show="deviceModes[device.name] === 'stop'" class="w-2.5 h-2.5 rounded-full bg-red-500" style="box-shadow:0 0 6px rgba(239,68,68,0.6)" title="Stopped"></span>
            ...
            <div class="flex items-center gap-2 mb-2">
              <div class="flex-shrink-0 rounded flex items-center justify-center"
                :class="device.type === 'ulanzi' ? 'bg-amber-500/10' : 'bg-cyan-500/10'"
                :style="device.type === 'ulanzi' ? 'width:22px;height:12px' : 'width:16px;height:16px'">
                <span x-html="icon(device.type === 'ulanzi' ? 'tv' : 'monitor', device.type === 'ulanzi' ? 10 : 11, device.type === 'ulanzi' ? 'color:#f59e0b' : 'color:#22d3ee')" style="display:flex"></span>
              </div>
              <span class="device-badge"
                :class="device.type === 'ulanzi' ? 'text-ulanzi bg-amber-500/10' : 'text-pixoo bg-cyan-500/10'"
                x-text="device.type"></span>
              <span class="text-[11px] font-mono leading-none" style="color:rgba(255,255,255,0.3)" x-text="device.name"></span>
            </div>
            <div class="h-7 overflow-hidden">
              <div class="text-[11px] leading-snug" style="color:rgba(255,255,255,0.25);display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden"
                x-text="[device.comment, device.ip].filter(Boolean).join(' · ')"></div>
            </div>
          </div>
          ...
          <div class="p-5 flex flex-col gap-5">
            ...
            <div>
              <div class="flex items-center justify-between mb-2 gap-3">
                ...
                <div class="flex items-center gap-1.5 flex-shrink-0">
                  <!-- Stale: errors climbing but still under offline threshold -->
                  <span
                    x-show="deviceHealth[device.name] && deviceHealth[device.name].online && deviceHealth[device.name].consecutiveErrors > 0"
                    ...
                    :title="'render errors: ' + (deviceHealth[device.name]?.consecutiveErrors || 0) + '/5 — device may be flaky'">
                    <span x-text="'stale ' + (deviceHealth[device.name]?.consecutiveErrors || 0)"></span>
                  </span>
                  <!-- Offline: circuit-broken — click for recovery steps -->
                  <button
                    x-show="deviceHealth[device.name] && !deviceHealth[device.name].online"
                    @click="openRecoveryHelp(device.name)"
                    class="px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider transition"
                    style="background:rgba(239,68,68,0.15);color:#fca5a5;border:1px solid rgba(239,68,68,0.4);cursor:pointer"
                    onmouseover="this.style.background='rgba(239,68,68,0.25)'"
                    onmouseout="this.style.background='rgba(239,68,68,0.15)'"
                    title="Device offline — click for recovery steps">
                    offline
                  </button>
            ...
            <!-- Active scene -->
            <div>
              <div class="section-label">Active scene</div>
              <div class="flex gap-2 items-center">
                <select class="field flex-1 text-xs font-mono"
                  :value="device.scene || ''"
                  @change="changeScene(device.name, $event.target.value)"
                  style="padding:7px 38px 7px 10px">
                  <option value="">— none —</option>
                  <optgroup label="Custom Scenes">
                    <template x-for="key in customSceneKeys(device.type)" :key="key">
                      <option :value="key" :selected="device.scene === key" x-text="scenePrettyName(key)"></option>
                    </template>
                  </optgroup>
                  <optgroup label="Built-in">
                    <template x-for="mode in builtinModes(device.type)" :key="mode.key">
                      <option :value="mode.key" :selected="device.scene === mode.key" x-text="mode.label"></option>
                    </template>
                  </optgroup>
                </select>
                <button class="icon-button" x-show="device.scene && !device.scene.startsWith('builtin:')"
                  @click="openSceneSettings(device.name, device.scene)" title="Scene settings">
                  <span x-html="icon('settings', 14)" style="display:flex"></span>
                </button>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="flex gap-1.5">
                <button class="btn-mode flex-1" @click="setMode(device.name, 'play')" :disabled="!mqttConnected"
                  :style="deviceModes[device.name] === 'play' ? 'background:rgba(34,197,94,0.2);border-color:rgba(34,197,94,0.5);color:#86efac' : ''">
                ...
                <button class="btn-mode flex-1" @click="setMode(device.name, 'pause')" :disabled="!mqttConnected"
                  :style="deviceModes[device.name] === 'pause' ? 'background:rgba(245,158,11,0.2);border-color:rgba(245,158,11,0.5);color:#fcd34d' : ''">
                ...
                <button class="btn-mode flex-1" @click="setMode(device.name, 'stop')" :disabled="!mqttConnected"
                  :style="deviceModes[device.name] === 'stop' ? 'background:rgba(239,68,68,0.2);border-color:rgba(239,68,68,0.5);color:#fca5a5' : ''">
            ...
            <div>
              <div class="flex items-center justify-between mb-1.5">
                <div class="section-label mb-0">Brightness</div>
                <label class="flex items-center gap-1.5 cursor-pointer" :class="!mqttConnected ? 'opacity-30 pointer-events-none' : ''">
                  <span class="text-[10px] font-mono"
                    :style="deviceBrightness[device.name]?.override != null ? 'color:rgba(99,102,241,0.8)' : 'color:rgba(255,255,255,0.3)'"
                    x-text="deviceBrightness[device.name]?.override != null ? 'Override' : 'Auto'"></span>
                  <input type="checkbox" class="toggle toggle-xs toggle-primary"
                    :checked="deviceBrightness[device.name]?.override != null"
                    @change="(() => {
                      const on = $event.target.checked;
                      const val = deviceBrightness[device.name]?._sliderVal ?? 50;
                      setBrightness(device.name, on, val);
                      if (on && deviceBrightness[device.name]) deviceBrightness[device.name].override = val;
                      else if (deviceBrightness[device.name]) deviceBrightness[device.name].override = null;
                    })()">
                </label>
              </div>
              <div class="flex items-center gap-2">
                <input type="range" min="1" max="100" step="1"
                  ...
                  :class="deviceBrightness[device.name]?.override != null ? 'range-primary' : 'range-ghost opacity-40'"
                  :value="deviceBrightness[device.name]?.override ?? deviceBrightness[device.name]?.last ?? deviceBrightness[device.name]?._sliderVal ?? 50"
                  @input="if (deviceBrightness[device.name]) deviceBrightness[device.name]._sliderVal = parseInt($event.target.value)"
                  @change="(() => {
                    const v = parseInt($event.target.value);
                    if (deviceBrightness[device.name]) deviceBrightness[device.name]._sliderVal = v;
                    if (deviceBrightness[device.name]?.override != null) {
                      deviceBrightness[device.name].override = v;
                      setBrightness(device.name, true, v);
                    }
                  })()"
                  :disabled="!mqttConnected || deviceBrightness[device.name]?.override == null">
                <span class="text-xs font-mono w-10 text-right"
                  :style="deviceBrightness[device.name]?.override != null ? 'color:rgba(255,255,255,0.7)' : 'color:rgba(255,255,255,0.25)'"
                  x-text="(deviceBrightness[device.name]?.override != null
                    ? (deviceBrightness[device.name]?._sliderVal ?? deviceBrightness[device.name]?.override)
                    : (deviceBrightness[device.name]?.last ?? '—')) + '%'"></span>
              </div>
            </div>

            <!-- Footer actions -->
            <div class="flex gap-1.5 pt-4" style="border-top:1px solid rgba(255,255,255,0.06)">
              <button @click="clearOverlay(device.name)" x-show="mqttConnected"
                class="btn-mode flex-1"
                style="color:rgba(245,158,11,0.6)"
                onmouseover="this.style.cssText='color:#fcd34d;background:rgba(245,158,11,0.15);border-color:rgba(245,158,11,0.4);flex:1'"
                onmouseout="this.style.cssText='color:rgba(245,158,11,0.6);flex:1'">
                <span x-html="icon('rotate-ccw', 13)" style="display:flex;flex-shrink:0"></span>
                Reset
              </button>
              <button @click="saveToFile(device.name)" :disabled="!isDeviceDirty(device.name)"
                class="btn-mode flex-1"
                :style="isDeviceDirty(device.name)
                  ? 'color:rgba(34,197,94,0.8);border-color:rgba(34,197,94,0.3)'
                  : 'opacity:0.35'"
                onmouseover="if(!this.disabled)this.style.cssText='color:#86efac;background:rgba(34,197,94,0.15);border-color:rgba(34,197,94,0.5);flex:1'"
                onmouseout="this.style.cssText=''">
                <span x-html="icon(isDeviceDirty(device.name) ? 'save' : 'check', 13)" style="display:flex;flex-shrink:0"></span>
                <span x-text="isDeviceDirty(device.name) ? 'Save' : 'Saved'"></span>
              </button>
            </div>
  ...
  <div x-show="showDeviceSettings" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center p-4"
    ...
    <div class="glass-card rounded-2xl w-full max-w-lg p-6 flex flex-col gap-5" x-transition:enter="transition ease-out duration-200"
      ...
      <div class="flex gap-1.5 pt-1">
        <button @click="closeDeviceSettings()" class="btn-mode flex-1"
          style="color:rgba(255,255,255,0.5)"
          onmouseover="this.style.cssText='color:#fff;background:rgba(255,255,255,0.1);border-color:rgba(255,255,255,0.2);flex:1'"
          onmouseout="this.style.cssText='color:rgba(255,255,255,0.5);flex:1'">
          <span x-html="icon('x', 13)" style="display:flex;flex-shrink:0"></span>
          Close
        </button>
        <button @click="saveDeviceSettings()" class="btn-mode flex-1"
          style="color:rgba(34,197,94,0.8);border-color:rgba(34,197,94,0.3)"
          onmouseover="this.style.cssText='color:#86efac;background:rgba(34,197,94,0.15);border-color:rgba(34,197,94,0.5);flex:1'"
          onmouseout="this.style.cssText='color:rgba(34,197,94,0.8);border-color:rgba(34,197,94,0.3);flex:1'">
          <span x-html="icon('save', 13)" style="display:flex;flex-shrink:0"></span>
          Save
        </button>
      </div>
    </div>
  ...
  <!-- Recovery help modal — actionable steps when a device is offline -->
  <div x-show="showRecoveryHelp" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center p-4"
    style="background:rgba(0,0,0,0.72);backdrop-filter:blur(6px)" @click.self="closeRecoveryHelp()">
    <div class="glass-card rounded-2xl w-full max-w-xl p-6 flex flex-col gap-4" x-transition:enter="transition ease-out duration-200"
      x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" @click.stop>
      ...
      <div class="flex items-center justify-between gap-3">
        <div>
          <h2 class="font-semibold text-white flex items-center gap-2">
            ...
            Device offline: <span x-text="recoveryHelpDevice"></span>
          </h2>
          <div class="text-xs mt-1" style="color:rgba(255,255,255,0.45)"
            x-text="((deviceHealth[recoveryHelpDevice]?.ip) || '?') + ' · ' + ((deviceHealth[recoveryHelpDevice]?.type) || '?') + ' · ' + (deviceHealth[recoveryHelpDevice]?.consecutiveErrors || 0) + ' consecutive errors'"></div>
        </div>
        <button @click="closeRecoveryHelp()" class="btn-mode w-8 h-8 p-0 rounded-lg" style="display:flex;align-items:center;justify-content:center">
          <span x-html="icon('x', 15)" style="display:flex"></span>
        </button>
      </div>

      <div class="text-sm leading-relaxed" style="color:rgba(255,255,255,0.85)">
        Pixdcon's render loop is in the circuit-broken state for this device — it's been failing every render attempt. Try the steps below in order; the first one that applies fixes most cases.
      </div>
      ...
      <div class="flex flex-col gap-3">
        ...
        <div class="rounded-lg p-3" style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08)">
          <div class="font-semibold text-white text-sm mb-1">1. Power &amp; cable</div>
          <div class="text-xs" style="color:rgba(255,255,255,0.65)">
            Walk over and check: power LED lit? USB-C cable seated? Power brick warm? On the Ulanzi TC001, a dead brick is the most common failure — borrow a known-good 5V/2.4A brick to A/B test.
          </div>
        </div>

        <!-- Step 2 (Ulanzi/AWTRIX): AP-mode rescue -->
        <div x-show="deviceHealth[recoveryHelpDevice]?.type === 'ulanzi'"
          class="rounded-lg p-3" style="background:rgba(99,102,241,0.06);border:1px solid rgba(99,102,241,0.25)">
          <div class="font-semibold text-white text-sm mb-1">2. AP-mode rescue (most common after a brownout)</div>
          <div class="text-xs leading-relaxed" style="color:rgba(255,255,255,0.7)">
            If the device powered up but lost WiFi credentials, it's broadcasting an open access point. On your laptop:
            <ol class="mt-2 space-y-1.5 pl-4" style="list-style:decimal;color:rgba(255,255,255,0.8)">
              <li>
                Join the WiFi network
                <span class="font-mono px-1.5 py-0.5 rounded text-[11px]"
                  style="background:rgba(99,102,241,0.18);color:#a5b4fc"
                  x-text="deviceHealth[recoveryHelpDevice]?.apSsid || 'awtrix_…'"></span>
                in your WiFi menu (open network, no password).
                <span x-show="!deviceHealth[recoveryHelpDevice]?.apSsid" class="block mt-1 text-[10px]" style="color:rgba(255,255,255,0.45)">
                  Couldn't determine the exact suffix — pixdcon never got a successful poll. Look for any open <span class="font-mono">awtrix_*</span> SSID in your WiFi menu.
                </span>
              </li>
              <li>
                In your terminal:
                <div class="font-mono mt-1 px-2 py-1.5 rounded text-[11px] leading-relaxed"
                  style="background:rgba(0,0,0,0.35);color:#a5b4fc;border:1px solid rgba(99,102,241,0.15);user-select:all">cd ~/Code/pixdcon &amp;&amp; ./scripts/awtrix-rescue.sh <span x-text="deviceHealth[recoveryHelpDevice]?.ip || ''"></span></div>
              </li>
              <li>Script auto-pushes your home WiFi credentials (from agenix) and pings until the device joins the LAN. Pixdcon's liveness probe reclaims it within 30s.</li>
            </ol>
          </div>
        </div>

        <!-- Step 2 (Pixoo64): HTTP-stack hang power-cycle -->
        <div x-show="deviceHealth[recoveryHelpDevice]?.type === 'pixoo'"
          class="rounded-lg p-3" style="background:rgba(34,211,238,0.06);border:1px solid rgba(34,211,238,0.25)">
          <div class="font-semibold text-white text-sm mb-1">2. HTTP-hang power-cycle (Pixoo64 known issue)</div>
          <div class="text-xs leading-relaxed" style="color:rgba(255,255,255,0.7)">
            Pixoo64's HTTP stack can hang after extended uptime — TCP refuses new connections while ICMP/ARP still respond. Fix is a brief power cycle.
            <ul class="mt-2 space-y-1.5 pl-4" style="list-style:disc;color:rgba(255,255,255,0.8)">
              <li>
                If <span class="font-mono">powerCyclePlugin</span> is configured for this device, pixdcon will smart-plug-cycle it automatically once the circuit-breaker opens. Wait ~1 minute and check.
              </li>
              <li>
                If not, walk over and unplug the device for ~10 s, then plug it back in. Pixdcon's liveness probe reclaims it within 30 s of the device booting back.
              </li>
              <li>
                Pixoo doesn't have AP-mode rescue (no captive portal) — only power cycle helps.
              </li>
            </ul>
          </div>
        </div>
        ...
        <!-- Step 3: Flash issue -->
        <div class="rounded-lg p-3" style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08)">
          <div class="font-semibold text-white text-sm mb-1">3. If AP-mode keeps recurring</div>
          <div class="text-xs" style="color:rgba(255,255,255,0.65)">
            Repeated WiFi-credential loss after reboot suggests NVS flash flakiness on the device itself, not the power supply. Check telemetry trends on
            <span class="font-mono px-1 rounded text-[10px]" style="background:rgba(0,0,0,0.3);color:rgba(255,255,255,0.6)">home/hsb1/pixdcon/<span x-text="recoveryHelpDevice"></span>/telemetry</span>
            for early-warning signals (uptime resets, <span class="font-mono">bat_raw</span> decline, RAM drift). RMA may be needed.
          </div>
        </div>
      </div>

      <div class="flex gap-1.5 pt-1">
        <button @click="closeRecoveryHelp()" class="btn-mode flex-1"
          style="color:rgba(255,255,255,0.5)"
          onmouseover="this.style.cssText='color:#fff;background:rgba(255,255,255,0.1);border-color:rgba(255,255,255,0.2);flex:1'"
          onmouseout="this.style.cssText='color:rgba(255,255,255,0.5);flex:1'">
          <span x-html="icon('x', 13)" style="display:flex;flex-shrink:0"></span>
  ...
  <div x-show="showSceneSettings" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center p-4"
    style="background:rgba(0,0,0,0.72);backdrop-filter:blur(6px)" @mousedown.self.prevent>
    <div class="glass-card rounded-2xl w-full max-w-5xl max-h-[92vh] p-6 flex flex-col gap-5 overflow-hidden">
      ...
      <div class="pr-1 flex flex-col gap-5" style="flex:1 1 auto;min-height:0;overflow-y:auto;overscroll-behavior:contain" @input.debounce.300ms="_scheduleAutoOverlay()" @change="_scheduleAutoOverlay()">
      ...
      <div class="scene-actions-stack pt-1 flex-shrink-0">
        <div class="flex gap-1.5">
          <button @click="clearSceneOverlay()" class="btn-mode flex-1"
            :disabled="!mqttConnected || !currentSceneSettings"
            style="color:rgba(245,158,11,0.6)"
            onmouseover="if(!this.disabled)this.style.cssText='color:#fcd34d;background:rgba(245,158,11,0.15);border-color:rgba(245,158,11,0.4);flex:1'"
            onmouseout="this.style.cssText='color:rgba(245,158,11,0.6);flex:1'">
            <span x-html="icon('rotate-ccw', 13)" style="display:flex;flex-shrink:0"></span>
            Reset
          </button>
          <button @click="saveSceneSettings()" :disabled="!currentSceneSettings || !isSceneSettingsDirty()"
            class="btn-mode flex-1"
            :style="isSceneSettingsDirty()
              ? 'color:rgba(34,197,94,0.8);border-color:rgba(34,197,94,0.3)'
              : 'opacity:0.35'"
            onmouseover="if(!this.disabled)this.style.cssText='color:#86efac;background:rgba(34,197,94,0.15);border-color:rgba(34,197,94,0.5);flex:1'"
            onmouseout="this.style.cssText=''">
            <span x-html="icon(isSceneSettingsDirty() ? 'save' : 'check', 13)" style="display:flex;flex-shrink:0"></span>
            <span x-text="isSceneSettingsDirty() ? 'Save' : 'Saved'"></span>
          </button>
        </div>
        <details>
      ...
      return {
        ...
        async init() {
          try {
            ...
            setInterval(() => this.refreshScenes(), 10000);
          } catch (e) {
        ...
        isSceneSettingsDirty() {
          const current = this.sceneFormPayload();
          const saved = this._sceneSettingsSaved || {};
          return JSON.stringify(current) !== JSON.stringify(saved);
        },

        isDeviceDirty(name) {
          const state = this.sceneSettingsState?.[name] || {};
          for (const [scene, s] of Object.entries(state)) {
            if (JSON.stringify(s.effective || {}) !== JSON.stringify(s.saved || {})) return true;
          }
          return false;
        },

        _scheduleAutoOverlay() {
          if (this._autoOverlayTimer) clearTimeout(this._autoOverlayTimer);
          this._autoOverlayTimer = setTimeout(() => this._doAutoOverlay(), 300);
        },

        async _doAutoOverlay() {
          if (!this.currentSceneSettings || !this.mqttConnected) return;
          const { deviceName, sceneName } = this.currentSceneSettings;
          try {
            await fetch('/api/scene-settings/overlay', {
        ...
        _loadSceneSettingsForm(deviceName, sceneName) {
          ...
          this._sceneSettingsSaved = JSON.parse(JSON.stringify(state.saved || {}));
        ...
        async fetchStatus() {
          try {
            ...
            this.mqttConnected = data.mqttConnected;
            this.deviceModes = data.deviceModes || {};
            this.deviceHealth = data.deviceHealth || {};
            // Merge brightness — preserve local slider state
            for (const [name, bri] of Object.entries(data.deviceBrightness || {})) {
              const prev = this.deviceBrightness[name] || {};
              this.deviceBrightness[name] = {
                ...bri,
                _sliderVal: prev._sliderVal ?? bri.override ?? bri.last ?? 50,
              };
            }
          } catch {}
        },

        openRecoveryHelp(deviceName) {
          this.recoveryHelpDevice = deviceName;
          this.showRecoveryHelp = true;
        },

        closeRecoveryHelp() {
          this.showRecoveryHelp = false;
          this.recoveryHelpDevice = null;
        },
        ...
        previewLabel(device) {
          ...
          if (!preview) return '';
          const ts = Date.parse(preview.updatedAt || 0);
        ...
        async refreshScenes() {
          try {
            ...
            if (data.builtins) {
              ...
              // Store scene meta without builtins key
              const { builtins, ...meta } = data;
              this.sceneMeta = meta;
            } else {
              this.sceneMeta = data;
            }
        ...
        customSceneKeys(deviceType) {
          return Object.keys(this.config?.scenes || {}).filter(key => {
            const m = this.sceneMeta?.[key];
            return !m?.deviceType || m.deviceType === deviceType;
          });
        },

        builtinModes(deviceType) {
          return this.builtins[deviceType] || [];
        },

        async changeScene(deviceName, scene) {
          try {
            const r = await fetch('/api/scene', {
              method: 'POST', headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ deviceName, scene: scene || null }),
            });
            const data = await r.json();
            if (data.ok) {
              const dev = this.config?.devices?.find(d => d.name === deviceName);
              if (dev) dev.scene = scene || null;
              this.notify(deviceName + ': ' + (scene || 'none'), 'success');
            } else {
        ...
        async applyOverlay(name) {
          try {
            const device = this.config?.devices?.find(d => d.name === name);
            const r = await fetch('/api/overlay', {
              method: 'POST', headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ deviceName: name, scene: device?.scene || null }),
            });
            const data = await r.json();
        ...
        async setBrightness(name, enabled, value) {
          try {
            const r = await fetch('/api/brightness', {
              method: 'POST', headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ deviceName: name, enabled, value: Math.round(value) }),
            });
            const data = await r.json();
            if (!data.ok) this.notify(data.error || 'Failed', 'error');
          } catch (e) { this.notify(e.message, 'error'); }
        },
