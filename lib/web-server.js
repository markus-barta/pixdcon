          ...
          <div class="p-5 flex flex-col gap-5">
            ...
            <div class="flex gap-2 pt-4" style="border-top:1px solid rgba(255,255,255,0.06)">
              <button @click="clearOverlay(device.name)" x-show="mqttConnected"
                class="btn btn-sm btn-ghost flex-1 flex items-center justify-center gap-1.5 opacity-50 hover:opacity-100">
                <span x-html="icon('rotate-ccw', 13)" style="display:flex"></span>
                Reset
              </button>
              <button @click="saveToFile(device.name)" :disabled="!isDeviceDirty(device.name)"
                class="btn btn-sm flex-1 flex items-center justify-center gap-1.5"
                :class="isDeviceDirty(device.name) ? 'btn-primary' : 'btn-ghost opacity-50'">
                <span x-html="icon(isDeviceDirty(device.name) ? 'save' : 'check', 13)" style="display:flex"></span>
                <span x-text="isDeviceDirty(device.name) ? 'Save Settings' : 'Settings Saved'"></span>
              </button>
            </div>
    ...
    <div class="glass-card rounded-2xl w-full max-w-5xl max-h-[92vh] p-6 flex flex-col gap-5 overflow-hidden" @click.stop>
      ...
      <div class="scene-actions-stack pt-1">
        <div class="flex gap-2">
          <button @click="clearSceneOverlay()" class="btn btn-sm btn-ghost flex-1 flex items-center justify-center gap-1.5 opacity-50 hover:opacity-100"
          ...
          <button @click="saveSceneSettings()" :disabled="!currentSceneSettings || !isSceneSettingsDirty()"
            ...
            <span x-html="icon(isSceneSettingsDirty() ? 'save' : 'check', 13)" style="display:flex"></span>
            <span x-text="isSceneSettingsDirty() ? 'Save Settings' : 'Settings Saved'"></span>
          </button>
        </div>
        <details>
