...
<body class="min-h-screen" x-data="app()" x-init="init()" x-cloak>
            ...
            <div>
              ...
              <div class="flex items-center gap-2">
                ...
                <span class="text-xs font-mono w-10 text-right"
                  ...
                  x-text="(deviceBrightness[device.name]?.override ?? deviceBrightness[device.name]?.last ?? '—') + '%'"></span>
              </div>
            </div>
  ...
  <div x-show="showSceneSettings" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center p-4"
    ...
    <div class="glass-card rounded-2xl w-full max-w-5xl max-h-[92vh] p-6 flex flex-col gap-5 overflow-hidden" @click.stop>
      ...
      <div class="overflow-auto pr-1 flex flex-col gap-5" @input.debounce.300ms="_scheduleAutoOverlay()" @change="_scheduleAutoOverlay()">
  ...
  <script>
    ...
    function app() {
      return {
        ...
        isDeviceDirty(name) {
          // Check if any scene overlay differs from saved for this device
          const state = this.sceneSettingsState?.[name] || {};
          for (const [scene, s] of Object.entries(state)) {
            if (JSON.stringify(s.overlay || {}) !== '{}') return true;
