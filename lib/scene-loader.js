...
export class SceneLoader {
  ...
  async clearScene(sceneName) {
    // Record the cache-bust token FIRST, unconditionally. A scene registered in
    // config.json but not yet assigned to a device has no cache entries, yet its
    // file can still change — if we returned before setting the token, the next
    // load() would import the bare path and Node would serve a stale ESM module.
    // (PIXD-39: that stale module then renders while getSceneMetadata(), which
    // always cache-busts with ?meta=, reports the new metadata — so the UI and
    // the panel disagree and it looks like anything but a caching bug.)
