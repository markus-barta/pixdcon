...
export class MqttService {
  constructor(options = {}) {
    ...
    this.baseTopic = options.baseTopic || "home/hsb1/pixdcon";
   ...
   * @param {string} pattern - e.g. "home/hsb1/pixdcon/overlay/device/+/scenes"
   ...
   *
   * Subscription namespace includes the deviceName so that two devices running
   * the same scene get independent callback slots in `topicEntry.callbacks`
   * (which is keyed by namespace). Without this, the second device's subscribe
   * call would evict the first device's callback, leaving its `_state` frozen
   * at the initial retained burst.
   *
   * @param {string} sceneName
   * @param {string|null} deviceName
   */
  getSceneContext(sceneName, deviceName = null) {
    const ns = deviceName ? `${sceneName}__${deviceName}` : sceneName;
    return {
      deviceName,
      subscribe: (topic, cb) => this.subscribe(ns, topic, cb),
      subscribeWildcard: (topicPattern, cb) =>
        this.subscribeWildcard(ns, topicPattern, (fullTopic, payload) =>
          cb(payload, fullTopic),
        ),
      unsubscribeAll: () => this.unsubscribeAll(ns),
    };
  }
